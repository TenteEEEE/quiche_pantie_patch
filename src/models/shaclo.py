...
class patcher(patcher):
    def __init__(self, body="./body/body_shaclo.png", **options):
        ...
        try:
            ...
            self.stitch_correction = self.ask(
                question="Apply stitch correction? Usually not necessary.", default=False
            )
    ...
    def convert(self, image):
        ...
        out = skt.resize(
            pantie_new,
            (int(pantie_new.shape[0] * mag_r), int(pantie_new.shape[1] * mag_c)),
            anti_aliasing=True,
            mode="reflect",
        )
