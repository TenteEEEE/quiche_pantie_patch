...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Set the pantie number. When you set with --all, it is the start number [Default: Latest]",
)
...
parser.add_argument(
    "--disable_ribbon", action="store_true", default=False, help="Disable the center ribbon paint [Default: Enable]"
)
parser.add_argument(
    "--disable_decoration",
    action="store_true",
    default=False,
    help="Disable the decoration such as frill and lace [Default: Enable]",
)
parser.add_argument(
    "--disable_shading", action="store_true", default=False, help="Disable the automatic shading [Default: Enable]"
)
parser.add_argument(
    "--disable_texture",
    action="store_true",
    default=False,
    help="Disable the automatic base texture painting [Default: Enable]",
)
...
for fname in panties:
    ...
    if args.disable_ribbon is False:
        ...
        convined_texture[rib_r : rib_r + ribbon_bi.shape[0], rib_c : rib_c + ribbon_bi.shape[1], :3] = alpha_brend(
    ...
    if args.pad:
        ...
        print(pady)
        io.imsave(
            "./converted/bra/" + fname, np.pad(convined_texture, [(0, pady), (padx, padx), (0, 0)], mode="constant")
        )
    else:
