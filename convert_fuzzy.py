...
def convert2fuzzy(fname=None):
    ...
    if fname in panties:
        ...
        shifter_row = +(np.sin(np.linspace(0, 1 * np.pi, src.shape[0])+np.pi/8)*50)
        ...
        patch = skt.rotate(patch,90)[:,60:160]
        
        pantie[-250:,485:,:] = 0
        patch = skt.resize(patch[:,:,:],(patch.shape[0]-10,82),anti_aliasing=True,mode='reflect')
        [pr,pc,d] = patch.shape
        pantie[127-8:127-8+pr,:pc,:] = np.uint8(patch*255)
